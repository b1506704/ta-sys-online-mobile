...
export class UserStore extends StateService<UserState> {
  constructor(
    ...
    this.$isLoggedIn.subscribe((data: any) => {
      if (data === true) {
        ...
        this.store.setCurrentUserRole(this.getRoleId());
      }
    });
  }
  /**
   ...
   * @author Le Bao Anh
   * @version 1.0.0
   * @param startIndex - The current page of ss pagination
   * @param endIndex - The page size of ss pagination
   * @param sourceArray - The source array/state in a specific store service
   * @param addedArray - The array of items received from ss pagination
   * @return Return an array with filled items from ss pagination
   * @example
   * this.setState({
  ...
  fillEmpty(
    ...
    const result: Array<User> = sourceArray;
  ...
  uploadUser(user: User, page: number, size: number) {
    ...
    this.userService.uploadUser(user).subscribe({
      next: (data: any) => {
        ...
        // this.loadDataAsync(page, size);
        this.setIsLoading(false);
        this.store.showNotif(data.message, 'primary');
      },
      error: (data: any) => {
        this.setIsLoading(false);
        this.store.showNotif(data.message, 'danger');
        console.log(data);
      },
    });
    // }
  ...
  updateUser(user: User, key: string, page: number, size: number) {
    ...
    this.userService.updateUser(user, key).subscribe({
      next: (data: any) => {
        ...
        this.loadDataAsync(page, size);
        this.setIsLoading(false);
        this.store.showNotif(data.message, 'success');
      },
      error: (data: any) => {
        this.setIsLoading(false);
        this.store.showNotif(data.message, 'danger');
  ...
  loginUser(user: any) {
    ...
    this.userService.loginUser(user).subscribe({
      next: (data: any) => {
        ...
        this.store.setCurrentUserRole(data.roleId);
  ...
  logoutUser(user: User) {
    ...
    this.store.showNotif('Logout successfully', 'primary');
    //   },
    //   error: (data: any) => {
    //     this.setIsLoading(false);
    //     //
